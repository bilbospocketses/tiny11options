...
# elevated rights. We use it to perform the /Cleanup-Image /StartComponentCleanup
# /ResetBase that we cannot do offline because 25H2+ ISOs ship with CBS pre-staged
# Windows Update content marked "pending until reboot" — every offline /Cleanup-Image
# variant fails with 0x800f0806 (CBS_E_PENDING) until that state resolves at first boot.
# DISM /Cleanup-Image /RevertPendingActions only schedules the revert ("Revert of
# pending actions will be attempted after the reboot"); it can't execute offline.
#
...
:: Runs once at first boot via SetupComplete.cmd contract. Performs the
:: /Cleanup-Image /StartComponentCleanup /ResetBase that couldn't run offline
:: due to CBS pre-staged Windows Update content in 25H2+ image baselines.
