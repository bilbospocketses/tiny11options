...
# elevated rights. Two responsibilities:
#
...
#
#   2. Re-disable Windows Update services (wuauserv, UsoSvc, WaaSMedicSvc). Windows
#      OOBE/post-OOBE actively flips wuauserv back to Manual (Start=3) even after our
#      offline reg writes (proven Start=4 in the image hive) AND our RunOnce keys
#      (proven to have fired, since they self-cleared by Phase 7 C3 verification).
#      SetupComplete.cmd runs late enough in Setup that this re-disable has the best
#      chance of sticking. Uses sc config + reg add together as redundant mechanisms
#      so whichever Windows respects more wins.
#
# The script logs to %SystemDrive%\Windows\Logs\tiny11-postboot.log and self-deletes
# after running so it doesn't fire on any subsequent Setup pass (e.g. an OS repair).
...
# Installs the SetupComplete.cmd script into the mounted image at
# <mount>\Windows\Setup\Scripts\SetupComplete.cmd. Creates the parent directory
# if missing. Writes with CRLF line endings and ASCII encoding (cmd.exe rejects
...
function Install-Tiny11CorePostBootCleanup {
    ...
    $scriptPath = Join-Path $scriptDir 'SetupComplete.cmd'

    if (-not (Test-Path -LiteralPath $scriptDir)) {
        New-Item -ItemType Directory -Path $scriptDir -Force | Out-Null
    }

    $content = New-Tiny11CorePostBootCleanupScript
    # Normalize to CRLF — cmd.exe is forgiving but Windows Setup tooling expects CRLF.
    $crlfContent = ($content -split "`r?`n") -join "`r`n"
    [System.IO.File]::WriteAllText($scriptPath, $crlfContent, [System.Text.Encoding]::ASCII)

    Write-Verbose "Installed post-boot cleanup script at $scriptPath"
}
