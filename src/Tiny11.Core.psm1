...
# Returns the SetupComplete.cmd script content. Microsoft Windows Setup runs this
# script once at first user-side boot, after OOBE completes, in SYSTEM context with
# elevated rights. Four responsibilities:
#
#   1. /Cleanup-Image /StartComponentCleanup /ResetBase — finishes the offline-image
#      cleanup that can't run offline on 25H2+ ISOs (CBS pre-staged "pending content"
#      blocks every offline variant with 0x800f0806; /RevertPendingActions only
#      schedules the revert for next boot rather than executing).
#
#   2. Re-disable Windows Update services (wuauserv, UsoSvc, WaaSMedicSvc) via
#      sc config + sc stop + reg add (redundant mechanisms). Handles the first-boot
#      window before the scheduled task fires (which has a 2-min boot delay).
#
#   3. Register the "tiny11options\Keep WU Disabled" scheduled task via
#      `schtasks /create /xml`. The task XML is at <same dir>\tiny11-wu-enforce.xml;
#      task references the script at <same dir>\tiny11-wu-enforce.ps1 by absolute path.
#
#   4. Run tiny11-wu-enforce.ps1 once immediately so the full WUB recipe re-applies
#      at first-boot time rather than waiting for the 2-min boot delay on the next reboot.
#
# The cmd script logs to %SystemDrive%\Windows\Logs\tiny11-postboot.log and self-deletes
# after running. The .ps1 and .xml stay — the scheduled task references the .ps1 by
# absolute path; deleting it would break recurring enforcement.
function New-Tiny11CorePostBootCleanupScript {
    [CmdletBinding()]
    param([switch]$IncludePostBootCleanupRegistration)

    # Heredoc is single-quoted to prevent PowerShell variable expansion of CMD's %VAR% tokens.
    $base = @'
@echo off
...
# Returns the tiny11-wu-enforce.ps1 script content. Installed alongside SetupComplete.cmd
# and registered as the "tiny11options\Keep WU Disabled" scheduled task. Runs at boot+5min,
# daily at 03:00, and on every Windows Update event (Microsoft-Windows-WindowsUpdateClient/
# Operational event ID 19). Idempotent — when state is already correct, every check is a
# Test-Path/Get-ItemProperty read-and-skip; fast common-case no-op.
function New-Tiny11CoreWuEnforceScript {
    [CmdletBinding()]
    param()

    @'
# tiny11-wu-enforce.ps1 — Re-applies the WUB recipe (service disable + scheduled-task
# removal + IFEO blocks + process kill) on every boot, every minute, daily, and on WU events.
# Generated by New-Tiny11CoreWuEnforceScript (src/Tiny11.Core.psm1).
#
...
# %SystemDrive%\Windows\Logs\tiny11-wu-enforce.log. Rotates to .log.1 when the active
# log reaches 5000 lines (~3 days at the PT1M cadence); prior .log.1 is dropped.
# Max disk footprint: 2 files * ~5000 lines (~6 days of history).

$ErrorActionPreference = 'Continue'
$logPath       = "$env:SystemDrive\Windows\Logs\tiny11-wu-enforce.log"
$logPathBackup = "$env:SystemDrive\Windows\Logs\tiny11-wu-enforce.log.1"
$logDir        = Split-Path -Parent $logPath
if (-not (Test-Path -LiteralPath $logDir)) { New-Item -ItemType Directory -Path $logDir -Force | Out-Null }

# Log rotation: if active log has >= 5000 lines, move it to .log.1 (replacing any
# prior backup). Caps disk footprint at 2 files. Rotation failures are swallowed
# silently — enforcement is the priority, logging is best-effort.
...
Write-EnforceLog '==== tiny11-wu-enforce triggered ===='

# --- 1. Service start types ---
# WUB Service_List target values: 4=Disabled, 3=Manual.
# UsoSvc=Manual per WUB (Disabled breaks Windows internals).
$serviceTargets = [ordered]@{
    'wuauserv'        = 4
    'dosvc'           = 4
    'WaaSMedicSvc'    = 4
    'UsoSvc'          = 3
...
foreach ($svc in $serviceTargets.Keys) {
    $svcKey = "HKLM:\SYSTEM\CurrentControlSet\Services\$svc"
    if (-not (Test-Path -LiteralPath $svcKey)) {
        Write-EnforceLog "  service $svc absent (offline-deleted; skipped)"
        continue
    }
    $expected = $serviceTargets[$svc]
    $current  = (Get-ItemProperty -LiteralPath $svcKey -Name Start -ErrorAction SilentlyContinue).Start
    if ($current -eq $expected) {
        Write-EnforceLog "  service $svc Start=$expected already"
    } else {
        try {
            Set-ItemProperty -LiteralPath $svcKey -Name Start -Value $expected -ErrorAction Stop
            Write-EnforceLog "  service $svc Start CORRECTED: $current -> $expected"
        } catch {
            Write-EnforceLog "  service $svc Start correction FAILED: $($_.Exception.Message)"
        }
    }
    if ($expected -eq 4) {
        ...
        if ($svcObj -and $svcObj.Status -eq 'Running') {
            try {
                ...
                Write-EnforceLog "  service $svc STOPPED (was running)"
            } catch {
                Write-EnforceLog "  service $svc stop FAILED: $($_.Exception.Message)"
            }
        }
    }
}

# --- 2. Scheduled task folders ---
$taskFolders = @(
...
foreach ($folder in $taskFolders) {
    $tasksPath = "$env:SystemRoot\System32\Tasks\$folder"
    if (Test-Path -LiteralPath $tasksPath) {
        try {
            Remove-Item -LiteralPath $tasksPath -Recurse -Force -ErrorAction Stop
            Write-EnforceLog "  task-folder $folder REMOVED (was recreated)"
        } catch {
            Write-EnforceLog "  task-folder $folder remove FAILED: $($_.Exception.Message)"
        }
    } else {
        Write-EnforceLog "  task-folder $folder absent (no-op)"
    }
}

# --- 3. IFEO Debugger entries ---
$ifeoExes = @(
    'WaaSMedic.exe','WaasMedicAgent.exe','Windows10Upgrade.exe','Windows10UpgraderApp.exe',
    'UpdateAssistant.exe','UsoClient.exe','remsh.exe','EOSnotify.exe','SihClient.exe',
    'InstallAgent.exe','MusNotification.exe','MusNotificationUx.exe','MoNotificationUx.exe'
)
$ifeoBase = 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options'
foreach ($exe in $ifeoExes) {
    $key = Join-Path $ifeoBase $exe
    if (-not (Test-Path -LiteralPath $key)) {
        try { New-Item -Path $key -Force -ErrorAction Stop | Out-Null }
        catch { Write-EnforceLog "  IFEO $exe key-create FAILED: $($_.Exception.Message)"; continue }
    }
    $current = (Get-ItemProperty -LiteralPath $key -Name Debugger -ErrorAction SilentlyContinue).Debugger
    if ($current -eq 'systray.exe') {
        Write-EnforceLog "  IFEO $exe Debugger=systray.exe already"
    } else {
        try {
            Set-ItemProperty -LiteralPath $key -Name Debugger -Value 'systray.exe' -Type String -ErrorAction Stop
            Write-EnforceLog "  IFEO $exe Debugger CORRECTED: '$current' -> 'systray.exe'"
        } catch {
            Write-EnforceLog "  IFEO $exe Debugger correction FAILED: $($_.Exception.Message)"
        }
    }
}

# --- 4. Process kill (catch anything that slipped through IFEO) ---
$killList = @('WaaSMedic','WaasMedicAgent','MusNotification','MusNotificationUx','MoNotificationUx','MoUsoCoreWorker','UsoClient')
foreach ($procName in $killList) {
    $procs = Get-Process -Name $procName -ErrorAction SilentlyContinue
    if ($procs) {
        try {
            $procs | Stop-Process -Force -ErrorAction Stop
            Write-EnforceLog "  process $procName KILLED ($($procs.Count) instance(s))"
        } catch {
            Write-EnforceLog "  process $procName kill FAILED: $($_.Exception.Message)"
        }
    }
}

Write-EnforceLog '==== tiny11-wu-enforce done ===='
'@
}

# Returns the tiny11-wu-enforce.xml scheduled task definition. Registers as
# "tiny11options\Keep WU Disabled". Five triggers:
#   1. BootTrigger PT2M delay — catches whatever Windows OOBE/post-OOBE does at boot
#   2. TimeTrigger PT1M Repetition — fires every 60 seconds forever; the guaranteed
#      enforcement mechanism. Phase 7 C4 empirically showed the event-driven trigger
#      (#5 below) doesn't reliably fire for wuauserv state changes even though those
#      events ARE logged to System; the TimeTrigger is the proven safety net.
#   3. CalendarTrigger daily 03:00 — catches mid-day restorations
#   4. EventTrigger WindowsUpdateClient/Operational EventID 19 — fires on CU install
#   5. EventTrigger System / Service Control Manager EventID 7040 — fires within
#      seconds of ANY service start type change (best-effort; see #2 above).
...
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    ...
    <Description>Re-applies the WUB recipe (service disable / scheduled-task removal / IFEO blocks / process kill) on every boot + daily + on WU events. Idempotent fast no-op when state is already correct.</Description>
    <URI>\tiny11options\Keep WU Disabled</URI>
  </RegistrationInfo>
  <Triggers>
    <BootTrigger>
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "C:\Windows\Setup\Scripts\tiny11-wu-enforce.ps1"</Arguments>
    </Exec>
  </Actions>
</Task>
'@
...
# Installs the three post-boot cleanup artifacts into the mounted image at
# <mount>\Windows\Setup\Scripts\:
#   - SetupComplete.cmd            (ASCII, CRLF — cmd.exe convention)
#   - tiny11-wu-enforce.ps1        (UTF-8 + BOM — PS 5.1 reads BOM-less as Windows-1252)
#   - tiny11-wu-enforce.xml        (UTF-16 LE + BOM — Task Scheduler convention)
...
function Invoke-Tiny11CoreBuildPipeline {
    ...
    try {
        ...
        # Phase 3.5: best-effort pre-pipeline revert of pending CBS actions.
        # On 25H2+ ISOs the actual revert can't run offline — DISM responds "Revert of pending
        # actions will be attempted after the reboot" — so this call doesn't clear inherited
        # pending state on those builds. We retain it because: (a) it's cheap (~4s); (b) on
        # older images (Win10, Win11 23H2 and earlier) where pending state IS uncommitted-since-
        # last-boot, the revert may actually execute; (c) it provides a clean DISM session
        # marker before the heavy /Remove-Package work. The real cleanup for 25H2+ now happens
        # post-boot via SetupComplete.cmd injected at Phase 18 (Install-Tiny11CorePostBootCleanup).
        & $ProgressCallback @{ phase='preflight-revert-pending'; step='Reverting pending CBS actions (best-effort)'; percent=13 }
        ...
        if ($revertResult.ExitCode -ne 0) {
            Write-Warning "DISM /Cleanup-Image /RevertPendingActions returned non-zero (exit $($revertResult.ExitCode)). Best-effort only; SetupComplete.cmd at Phase 18 handles cleanup post-boot."
        }
        ...
        # Phase 18: inject-postboot-cleanup (replaces upstream lines 478-480 offline /Cleanup-Image).
        # Reasoning: 25H2+ ISOs ship with CBS pre-staged Windows Update content (CU 26200.NNNN)
        # that all offline /Cleanup-Image variants reject with 0x800f0806 (CBS_E_PENDING) — the
        # CBS log diagnoses this as "Pending content found, aborting maintenance" and only
        # resolves at first boot. /RevertPendingActions only schedules the revert for next boot;
        # it can't execute offline. So we inject SetupComplete.cmd that runs the equivalent
        # /Cleanup-Image online during first-boot OOBE completion, in SYSTEM context, when CBS
        # pending state has resolved naturally. The script self-deletes after running and logs
        # to %SystemDrive%\Windows\Logs\tiny11-postboot.log.
...
Export-ModuleMember -Function `
    ...
    New-Tiny11CorePostBootCleanupScript, `
    New-Tiny11CoreWuEnforceScript, `
    New-Tiny11CoreWuEnforceTaskXml, `
    Install-Tiny11CorePostBootCleanup, `
