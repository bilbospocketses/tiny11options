...
### Open issue (Phase 1 VM smoke not passing)
- Built ISO fails Win11 25H2 / 26H1 retail Setup with "Setup has failed to validate the product key." Vanilla upstream `tiny11maker.ps1` reproduces the same error on the same retail ISO via direct invocation, so the failure isn't fork-introduced. Community-cited working fix ([upstream issue #310 comment 2568435531](https://github.com/ntdevlabs/tiny11builder/issues/310#issuecomment-2568435531)) attributes the failure to invocation mode — invoking via `powershell -f <script>` from an admin Windows PowerShell terminal works; direct invocation / Run-with-PowerShell does not. Underlying cause appears to be a Setup file-handle / I/O context issue around reading the autounattend.xml at validation time, NOT the autounattend content. To be re-verified next session: vanilla upstream invoked via `powershell -f` against retail 25H2 ISO. If confirmed, our orchestrator may need an analogous invocation pattern (or a file-flush/encoding fix on `Set-Content`).
