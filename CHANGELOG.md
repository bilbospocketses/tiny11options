...
- Fast Build is now supported in Core ISO mode. `Invoke-Tiny11CoreBuildPipeline` gained a `[bool]$FastBuild = $false` parameter; when set, Phase 20 (`dism /Export-Image /Compress:max`) and Phase 22 (`/Compress:recovery` to install.esd) skip, saving roughly 20–40 minutes per Core build at the cost of a larger ISO (install.wim retains all original edition indices since Phase 20's per-index re-export is skipped). Windows Setup accepts the uncompressed install.wim, so the ISO boots and installs identically. Mirrors the Worker.psm1 fast-build pattern (lines 99-116). Pipeline emits `export-install-skip` and `export-install-esd-skip` progress markers when active. UI changes: removed the Step 1 auto-uncheck + greying + tooltip that previously disabled Fast Build when Core was selected; Step 3 build-mode summary now reflects all four mode combinations; Fast Build hint copy is mode-aware. `BuildHandlers.BuildCoreArgs` gained a `fastBuild` parameter and now emits `-FastBuild` when set; `tiny11Coremaker-from-config.ps1` gained a `[switch]$FastBuild` and forwards it to the pipeline.
