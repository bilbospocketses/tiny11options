...
- **`-ScratchDir`** — working directory the build pipeline uses for scratch space (DISM mount root, hive temp files, intermediate WIM exports). **Needs ~10 GB free.** Optional. **When invoking `tiny11maker.ps1` directly:** defaults to the script's own directory (`$PSScriptRoot`) — i.e. your repo clone, which usually isn't where you want a multi-GB write. Specify a `-ScratchDir <path>` explicitly to control where it lands. **When invoking via `tiny11options.exe`:** the GUI Step 1 auto-populates with `<UserDocuments>\tiny11_outputs\tiny11-<8charhex>` (v1.0.11+; was `%TEMP%\tiny11-<hex>` v1.0.9-v1.0.10) — you can replace the suggested path with your own choice or leave it for a one-shot scratch under the durable outputs folder. The headless CLI accepts `-ScratchDir <path>` like any other wrapper arg; the launcher then forwards your choice to the script.
- **`-OutputPath`** — output ISO path. Optional. **When invoking `tiny11maker.ps1` directly:** defaults to `<scratchDir>\tiny11.iso` (script-internal fallback). **When invoking via `tiny11options.exe`:** the GUI Step 1 auto-fills `<parent-of-scratchDir>\tiny11.iso` (or `tiny11core.iso` in Core mode) — pairing the new v1.0.11 scratch default with parent-of-scratch output yields `<UserDocuments>\tiny11_outputs\tiny11.iso` directly under the durable outputs folder.
